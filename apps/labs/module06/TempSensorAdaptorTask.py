...
import random
import logging
from labs.module06.MQTTClientConnector import MQTTCLientConnector as mqttClientConnector
...
#                 self.curTemp = self.SenseHat.get_temperature()  #get temperature from SenseHat
                ...
                if(abs(self.curTemp - self.sensorData.getAvgValue()) >= self.alertDiff):
                    logging.info('Excessive Temperature with a difference > ' + str(self.alertDiff) + 'Triggering Alert')
...
#                     self.connector.publishMessage('Temperature Alert message', self.sensorData.getSensorData())
                ...
                if (self.prevTempFlag == True):
                    if(self.prevTemp != self.curTemp): 
                        ...
                        self.mqttClientConnector.publishMeassage(self.sensorData)
                        sleep(5)
                        self.mqttClientConnector.on_disconnect("DeviceClient", self.sensorData , 0)
